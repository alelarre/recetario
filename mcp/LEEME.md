...
3. **Guardar el cliente y dar permiso**, pasándole el JSON bajado:

   ```sh
   npm run mcp:conectar -- ~/Downloads/client_secret_….json
   ```

   El client ID y el client secret quedan en el Llavero, en el ítem
   `recetario-mcp-cliente`; después el JSON se puede borrar.
...
| Código | Qué hacer |
|---|---|
| `sin-cliente` | El cliente no está en el Llavero, el JSON no es de un cliente de escritorio o Google no reconoce el cliente (se borró): repetir los pasos 1 a 3. |
