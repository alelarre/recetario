...
| **P17** | **La planilla de borradores se lee entera en cada dibujo** | `[2026-09-12]` `borradores.listar()` relee `_borradores` completa, sin copia en memoria: en cada dibujo del Recetario y de Ajustes —para el contador, así que abrir y cerrar el menú lateral son dos lecturas—, en cada dibujo de Borradores y de un borrador —incluidos tocar *Descartar*, *Cancelar* y *Editar*—, y antes de editar o descartar uno, para encontrar su fila. Es el mismo problema que tenía el `.md` en el modo cocina, que se resolvió leyendo una vez por pantalla. |
