...
| **P18** | **Una pantalla de arranque que muestre la comparación del índice** | `[2026-09-13]` Rehacer la pantalla de inicio para que muestre, con detalle, qué pasa al abrir: la copia local del índice contra la versión remota —fechas de modificación, cantidad de filas, si se usó la copia o se bajó la planilla, si hace falta reindexar—, y lo demás que el arranque verifica. El índice local ya está (P12): `store.arrancar()` sabe si usó la copia o bajó la planilla, y con qué fechas. **Primero presentar propuestas.** |
