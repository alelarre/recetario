...
| **P18** | **Una pantalla de arranque que muestre la comparación del índice** | `[2026-09-13]` Rehacer la pantalla de inicio para que muestre, con detalle, qué pasa al abrir: la copia local del índice contra la versión remota —fechas de modificación, cantidad de filas, si se usó la copia o se bajó la planilla, si hace falta reindexar—, y lo demás que el arranque verifica. Depende del índice local (el diseño en curso de P12). **Primero presentar propuestas.** |
| **P19** | **Que la carpeta de Drive y las categorías las defina el usuario** | `[2026-09-13]` Hoy la app busca la carpeta raíz por el nombre fijo `Recetario`, y la planilla y la cola por los nombres fijos `_indice` y `_borradores` (`src/config.ts`); no hardcodea ids. Las categorías son las subcarpetas, pero su color y su foto están atados al slug en el código (`src/ui/categorias.ts`, `tokens.css`, `src/categorias/`). La tarea: definir cómo sacar esas referencias fijas, dejar que el usuario elija su propia carpeta, y que pueda agregar y quitar categorías desde la app. Se superpone con P15, que resuelve lo mismo con un skill en vez de con la app: decidir juntas. |
| **P20** | **Un indicador visible de la versión de la app** | `[2026-09-13]` Mostrar qué versión está corriendo —semver, fecha de build, commit—, para saber si el celular ya tomó el último deploy. Hoy `package.json` dice `1.0.0` y nada lo muestra. Lugares candidatos: el pie del menú lateral, el pie del Recetario, o al lado del título «Recetario». |
| **P21** | **Los borradores con copia local, como el índice** | `[2026-09-13]` Refactorizar el manejo de `_borradores` para que replique el del índice local (P12, `docs/superpowers/specs/2026-09-13-indice-local-design.md`): una copia en el navegador, verificada al abrir comparando la metadata de la planilla con la guardada, que se baja sólo si difiere y se actualiza después de cada escritura —agregar, editar, descartar y convertir—, pidiendo la metadata nueva. Con la misma premisa: nunca hay escritura concurrente. Hoy P17 ya evita releer en cada dibujo, pero la planilla se sigue bajando entera al entrar a cada pantalla. **Depende de P12:** conviene hacerlo con ese diseño ya implementado, para reusar el mismo mecanismo en vez de escribirlo dos veces. |
