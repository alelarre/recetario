...
|---|---|---|
| **P1** | **El borrador se borra recién cuando la receta ya existe en Drive** | Hoy C01.7.1 dice que guardar la receta borra la fila del borrador «en la misma operación». Falta verificar qué hace el código realmente y fijar el orden: primero se crea el `.md` en Drive y se escribe su fila del índice; **sólo si las dos escrituras salieron bien** se borra el borrador. Si algo falla, el borrador queda: perder la captura porque la creación falló es el peor resultado posible de esa pantalla. |
| **P7** | **Editar receta: avisar de cambios sin guardar** | C04.1.1 ya lo pide —«salir sin guardar con cambios pendientes pregunta antes»— y no está implementado. Vale para el botón volver y para el gesto de atrás del sistema. Sin cambios, no pregunta nada. |
| **P8** | **Modo cocina: el paso 1 arranca seleccionado** | Al entrar por primera vez no hay paso actual, así que la pantalla no dice dónde estás. Si no hay ninguno elegido, el actual es el primero. |
| **P9** | **Modo cocina: que el texto del paso no se pueda seleccionar** | En Android, tocar un paso para marcarlo hecho selecciona el texto con muchísima frecuencia. `user-select: none` sobre el paso, y revisar que eso no rompa el copiar y pegar en otro lado de la pantalla. |
